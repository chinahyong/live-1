...
public class LiveHotRecommendAdapter extends PagerAdapter {
    ...
    public LiveHotRecommendAdapter(Context ctx) {
        this.context = ctx;
        imageHeight = (App.metrics.widthPixels - Utils.dpToPx(36)) / 2;
    }
    ...
    public Object instantiateItem(ViewGroup container, final int position) {
        ...
        View left = LayoutInflater.from(context)
            .inflate(R.layout.item_hot_official_recommended, linearLayout, false);
        LinearLayout.LayoutParams layoutParamLeft = new LinearLayout.LayoutParams(imageHeight,
            imageHeight);
        linearLayout.addView(left, layoutParamLeft);
        ImageView left_iv_photo = left.findViewById(R.id.item_iv_photo);
        TextView left_tv_tag = left.findViewById(R.id.item_tv_tag);
        TextView left_tv_online = left.findViewById(R.id.item_tv_online);
        TextView left_tv_title = left.findViewById(R.id.item_tv_title);
        AnchorBean anchorBeanLeft = recommends.get(2 * position);
        ImageLoaderUtil.getInstance()
            .loadImageCorner(context, left_iv_photo, anchorBeanLeft.headPic,
        ...
        if (recommends.size() >= 2 * (position + 1)) {
            View right = LayoutInflater.from(context)
                .inflate(R.layout.item_hot_official_recommended, linearLayout, false);
            LinearLayout.LayoutParams layoutParamRight = new LinearLayout.LayoutParams(imageHeight,
                imageHeight);
            ...
            ImageLoaderUtil.getInstance()
                .loadImageCorner(context, right_iv_photo, anchorBeanRight.headPic,
                    Constants.COMMON_DISPLAY_IMAGE_CORNER_2, CornerType.ALL);
        ...
        container.addView(linearLayout,
            new ViewGroup.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, imageHeight));
        return linearLayout;
    }
