...
public class LiveHotAdapter extends RecyclerView.Adapter {
    ...
    public LiveHotAdapter(Context ctx) {
        this.ctx = ctx;
        mImageViewHeight = App.metrics.widthPixels;
        viewPagerHeight = (App.metrics.widthPixels - Utils.dpToPx(36)) / 2;
    }
        ...
        mBannerView.setPages(new CBViewHolderCreator() {
            ...
            .setOnItemClickListener(new com.bigkoo.convenientbanner.listener.OnItemClickListener() {
                ...
                public void onItemClick(int position) {
                    MobclickAgent.onEvent(App.mContext, "clickbannerInIndex");
                    OperationHelper.onEvent(App.mContext, "clickbannerInIndex", null);
    ...
    private RecyclerView.ViewHolder onCreateBannerView(ViewGroup parent) {
        mBannerLayout = (LinearLayout) LayoutInflater.from(ctx)
            .inflate(R.layout.a_main_banner_layout, parent, false);
        mBannerView = (ConvenientBanner) mBannerLayout.findViewById(R.id.convenientBanner);
        // 设置横幅底部引导图片，并启动自动播放
        mBannerView.setPageIndicator(
            new int[]{R.drawable.icon_circle_focus_off, R.drawable.icon_circle_focus_on})
            .startTurning(TIME_INTERVAL);
        // 设置包含Banner的RelativeLayout大小
        int liLayoutWidth = App.metrics.widthPixels;
    ...
    private RecyclerView.ViewHolder onCreateRecommendView(ViewGroup parent) {
        //recommendView init
        mRecommendView = (LinearLayout) LayoutInflater.from(ctx)
            .inflate(R.layout.fragment_hot_recommend, parent, false);
    ...
    public int getItemCount() {
        return getBannerViewCount() + getHotViewCount() + getRecommendViewCount()
            + getFooterViewCount();
    }
    ...
    private int getRecommendViewCount() {
        //推荐数据要在热3、4之间显示
        return (getHotViewCount() == 0 || getHotViewCount() < 2 || mRecommendList.size() == 0) ? 0
            : 1;
    }
