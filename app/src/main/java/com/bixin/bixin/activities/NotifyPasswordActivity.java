...
public class NotifyPasswordActivity extends LZActivity {
        ...
        OnReceiveMsgListener loOnCheckVCode = new OnReceiveMsgListener() {
            ...
            public void onReceiveMsg(Message poMsg) {
                if (poMsg.what == MsgTypes.CHECK_REGISTER_VERIFY_CODE_SUCCESS) {
                    try {
                        Business.getBackLogPwd(NotifyPasswordActivity.this,
                            moEtNewPwd.getText().toString());
                    } catch (Exception e) {
                        moProgress.dismiss();
                        moProgress = null;
                        e.printStackTrace();
    ...
    private class OnSendVCode implements OnClickListener {
        ...
        public void onClick(View v) {
            ...
            moProgress = Utils.showProgress(NotifyPasswordActivity.this);
            try {
                Business.sendModifyVerifyCode(NotifyPasswordActivity.this, lsMobile);
            } catch (Exception e) {
                e.printStackTrace();
                moProgress.dismiss();
    ...
    private class OnSubmit implements OnClickListener {
        ...
        @Override
        public void onClick(View v) {
            OperationHelper.onEvent(App.mContext, "clickFinishInForgetPasswordPage", null);
            // 1 获取用户输入
            String lsMobile = moEtMobile.getText().toString().replaceAll("\\s+", "");
            ...
            // 3 验证验证码
            moProgress = Utils.showProgress(NotifyPasswordActivity.this);
            Business.checkRegisterVerifyCode(NotifyPasswordActivity.this, lsVCode);
        }
    }
