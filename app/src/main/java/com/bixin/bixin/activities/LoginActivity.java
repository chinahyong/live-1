...
import static com.bixin.bixin.App.mContext;
...
public class LoginActivity extends BaseFragmentActivity {
    ...
    public void initWidgets() {
        mLoginBg = findViewById(R.id.iv_login_bg);
        moLlLoginByQq = findViewById(R.id.login_ll_by_qq);
        mLoginByWeixin = findViewById(R.id.login_ll_by_weixin);
        mLoginByWeibo = findViewById(R.id.login_ll_by_weibo);
        mLoginByPhone = findViewById(R.id.login_ll_by_phone);
        mTvProtocal = findViewById(R.id.login_agree_protocal);
        mSurfaceView = findViewById(R.id.sv_login_bg);
        mp = new MediaPlayer();
    ...
    public void play()
        ...
        AssetFileDescriptor asset = this.getAssets().openFd("login_bgm.mp4");
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        if (!TextUtils.isEmpty(AppConfig.getInstance().wallPaper)) {
            ImageLoaderUtil.getInstance()
                .loadImageAndDefault(mLoginBg, AppConfig.getInstance().wallPaper,
                    R.drawable.login_bg2, R.drawable.login_bg2);
        }
    ...
    private UMAuthListener umAuthListener = new UMAuthListener() {
        ...
        public void onComplete(SHARE_MEDIA platform, int i, Map<String, String> value) {
            ...
            if (platform == SHARE_MEDIA.WEIXIN) {
                ...
                BusinessUtils
                    .loginByWeixin(LoginActivity.this, psAccessToken, psOpenId, refreshToken,
                        unionid, expiresIn, new WxCallbackDataHandle(LoginActivity.this));
            } else if (platform == SHARE_MEDIA.QQ) {
                // 3 QQ登录
                BusinessUtils.loginByQQ(LoginActivity.this, psAccessToken, psOpenId, expiresIn,
                    new WxCallbackDataHandle(LoginActivity.this));
                ...
                BusinessUtils.loginByWeibo(LoginActivity.this, accessToken, uid, expiresIn,
                    new WxCallbackDataHandle(LoginActivity.this));
            }
        }
    ...
    protected void handleMessage(Message msg) {
        ...
        switch (msg.what) {
            case MsgTypes.LOGIN_BY_QQ_SUCCESS:
                ...
                JPushInterface
                    .setAliasAndTags(LoginActivity.this, lsUid, null, new TagAliasCallback() {
                        @Override
                ...
                BusinessUtils
                    .getMyUserInfo(LoginActivity.this, new MyUserInfoCallbackDataHandle(mHandler));
    ...
    private void reportDeviceInfo() {
        reportRegisterId();
        // 上传设备号
        BusinessUtils.reportDeviceId(mActivity, TelephoneUtil.getDeviceId(mActivity),
            TelephoneUtil.getMacAddress(mActivity), TelephoneUtil.getDeviceImei(mActivity));
        // 同盾初始化
        try {
    ...
    private void loginSuccessJump() {
        ...
        EvtLog.e(TAG, "CurTime:" + mCurLauchTime + "LastTime:" + mLastLauchTimes + "interval:"
            + mobileBindInterval);
            ...
            switch (v.getId()) {
                ...
                case R.id.login_ll_by_weixin:
                    ...
                    if (!umShareAPI.isInstall(LoginActivity.this, SHARE_MEDIA.WEIXIN)) {
                        Toast.makeText(getApplicationContext(), R.string.uninstall_weixin_tip,
                            Toast.LENGTH_SHORT).show();
                        return;
                    }
                    umShareAPI
                        .deleteOauth(LoginActivity.this, SHARE_MEDIA.WEIXIN, new UMAuthListener() {
                            @Override
                            public void onStart(SHARE_MEDIA share_media) {
                            ...
                            public void onComplete(SHARE_MEDIA share_media, int i,
                                ...
                                umShareAPI.doOauthVerify(LoginActivity.this, SHARE_MEDIA.WEIXIN,
                                    umAuthListener);
                            }
                ...
                case R.id.login_agree_protocal:
                    ...
                    webInfo.put(WebViewActivity.URL,
                        WebConstants.getFullWebMDomain(WebConstants.REGISTER_PROTOCOL));
                    webInfo.put(WebViewActivity.IS_NOT_SHARE, String.valueOf(true));
                    ActivityJumpUtil.gotoActivity(LoginActivity.this, WebViewActivity.class, false,
