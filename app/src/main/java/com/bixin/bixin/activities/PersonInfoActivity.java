...
import com.bixin.bixin.user.act.EditAlbumActivity;
...
import com.bixin.bixin.adapters.PersonAlbumAdapter;
import com.bixin.bixin.adapters.PersonCenterAdapter;
...
public class PersonInfoActivity extends BaseFragmentActivity implements OnClickListener,
     ...
     * 头像、认证标志、性别图标、背景
     */
    private ImageView moIvPhoto, moIvPhotoV, mIvUserSex, mTopBackgroup, mIvBottomFocus;
    /**
     * 昵称、ID 、动态、粉丝、关注、动态数、粉丝数、关注数、认证信息、个人签名、topBar标题
     */
    private TextView moTvNickname, moTvUserId, mTvVerifyInfo, mTvBarTitle, mTvAge, mTvConstellation;
    // 头部左侧按钮,右侧按钮
    ...
    private void startPhotoAnimation() {
        PropertyValuesHolder propertyValuesHolder = PropertyValuesHolder
            .ofFloat(View.TRANSLATION_Y, 0, -Utils.dip2px(mActivity, 232), 0);
        ObjectAnimator objectAnimator = ObjectAnimator
            .ofPropertyValuesHolder(mHeadLivePhoto, propertyValuesHolder);
    ...
    protected void initData(Bundle savedInstanceState) {
        ...
        if (intent != null) {
            mPersonInfo = (Map<String, String>) intent
                .getSerializableExtra(PersonInfoActivity.PERSON_INFO);
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.item_fans_layout:
                ActivityJumpUtil
                    .toUserFansActivity(mActivity, mPersonUid, isOwer, REQUEST_CODE_FLUSH_ACTIVITY);
                break;
            case R.id.item_focus_layout:
                ActivityJumpUtil.toUserFocusActivity(mActivity, mPersonUid, isOwer,
                    REQUEST_CODE_FLUSH_ACTIVITY);
                break;
            case R.id.item_guard_layout:
                ActivityJumpUtil.toWebViewActivity(mActivity,
                    WebConstants.getFullWebMDomain(WebConstants.WEB_USER_GUARD_LIST_URL)
                        + mPersonUid, true, 0);
                break;
            ...
            case R.id.my_info_level_layout:
                ...
                webInfo.put(WebViewActivity.URL,
                    WebConstants.getFullWebMDomain(WebConstants.COMMON_LEVEL_URL));
            ...
            case R.id.person_focus_layout:
                MobclickAgent.onEvent(App.mContext, "followInpersonalPage");
                // 如果已经关注了，则取消关注
                if (Utils.strBool(mPersonInfo.get("isAttention"))) {
                    BusinessUtils.removeFollow(mActivity,
                        new RemoveFollowCallbackData(PersonInfoActivity.this), mPersonUid);
                } else {
                    OperationHelper.onEvent(App.mContext, "followInpersonalPage", null);
                    BusinessUtils.follow(mActivity, new FollowCallbackData(PersonInfoActivity.this),
                        mPersonUid);
                }
                break;
            ...
            case R.id.item_live_photo_layout:
                MobclickAgent.onEvent(App.mContext, "liveInpersonalPage");
    ...
    protected void handleMessage(Message msg) {
        switch (msg.what) {
            case MsgTypes.GET_USER_INFO_SUCCESS:
                ...
                initHeadData();
                try {
                    mAlbumBeans = JacksonUtil
                        .readValue(mPersonInfo.get("gallery"), List.class, AlbumBean.class);
            ...
            case MsgTypes.FOLLOW_SUCCESS:
                //关注当前用户
                OperationHelper.onEvent(App.mContext, "followInpersonalPageSuccessful", null);
                mIsAttention = true;
                /** 返回关注此用户的状态*/
            ...
            case MsgTypes.REMOVE_FOLLOW_SUCCESS:
                //取消关注当前用户
                OperationHelper
                    .onEvent(App.mContext, "clickCancelFollowBroadcasterInpersonalPage",
                        null);
                mIsAttention = false;
                /** 返回关注此用户的状态*/
    ...
    private void initUI(LayoutInflater inflater) {
        ...
        mTopMargin = (int) (App.metrics.widthPixels * 0.7f - getResources()
            .getDimension(R.dimen.layout_top_title_bg_height));
        RelativeLayout.LayoutParams layoutParams = (RelativeLayout.LayoutParams) mTopBackgroup
            .getLayoutParams();
        layoutParams.topMargin = -mTopMargin;
        layoutParams.height = (int) (App.metrics.widthPixels * 0.7f);
    ...
    private void initHeadLayout() {
        ...
        mIconPageIndicator = (IconPageIndicator) mHeadLayout
            .findViewById(R.id.person_info_album_indicator);
     ...
     * 初始化礼物数据(热门、守护礼物）'
     */
    private void initAlbumListData(ViewPager viewPager, IconPageIndicator indicator,
        List<AlbumBean> giftsData) {
        ...
        if (giftsData.size() > 0 && giftsData.size() <= 4) {
            scale = (App.metrics.widthPixels - Utils.dip2px(mActivity, 5) * 5) / 4;
            indicator.setVisibility(View.GONE);
        } else if (giftsData.size() > 4 && giftsData.size() <= 8) {
            // 图片高度 + 图片上下间距
            scale = (App.metrics.widthPixels - Utils.dip2px(mActivity, 5) * 5) / 4 * 2 + Utils
                .dip2px(mActivity, 5);
            indicator.setVisibility(View.GONE);
        ...
        // 有相册列表
        if (scale > 0) {
            mListView.getRefreshableView().setHeaderViewSize(App.metrics.widthPixels,
                (int) (App.metrics.widthPixels * 0.66 + scale));
        } else {
            //无相册列表
            mListView.getRefreshableView().setHeaderViewSize(App.metrics.widthPixels,
                (int) (App.metrics.widthPixels * 0.53));
        }
        viewPager.setAdapter(
            new IconPageAdapter<>(giftsViewData, R.drawable.bg_person_indicator_selector));
    ...
    private void initHeadData() {
        ...
        if (!TextUtils.isEmpty(lsPhoto)) {
            ...
            ImageLoaderUtil.getInstance()
                .loadHeadPic(mActivity.getApplicationContext(), moIvPhoto, lsPhoto);
            ImageLoaderUtil.getInstance()
                .loadImageCorner(mActivity.getApplicationContext(), mHeadLivePhoto, lsPhoto,
                    Constants.COMMON_DISPLAY_IMAGE_CORNER_2, CornerType.ALL);
        }
        moIvPhotoV.setVisibility(
            Utils.getBooleanFlag(mPersonInfo.get("verified")) ? View.VISIBLE : View.GONE);
        ...
        if (!TextUtils.isEmpty(birthday)) {
            ...
            if (birthdays.length == 3) {
                ...
                // 星座获取,显示
                int constellation = DateUtil
                    .getConstellation(Integer.valueOf(birthdays[1]), Integer.valueOf(birthdays[2]));
        ...
        if (!TextUtils.isEmpty(medals)) {
            ...
            try {
                ...
                if (count < 10) {
                    for (int i = 0; i < medalArray.length(); i++) {
                        ...
                        if (!TextUtils.isEmpty(url)) {
                            spannableStringBuilder.append(" ")
                                .append(Utils.getImageToSpannableString(mTvModels, url, size));
                        }
                    }
                    //主播等级
                    if (!TextUtils.isEmpty(moderatorLevel)) {
                        //勋章 + 主播等级 < 10 显示第一行
                        if (++count <= 10) {
                            spannableStringBuilder.append(" ").append(Utils
                                .getImageToSpannableString(mTvModels, Utils
                                    .getLevelImageResourceUri(Constants.USER_ANCHOR_LEVEL_PIX,
                                        moderatorLevel), size));
                        } else {
                            // 否则  显示到第二行
                            spannableStringBuilder2.append(" ").append(Utils
                                .getImageToSpannableString(mTvModels2, Utils
                                    .getLevelImageResourceUri(Constants.USER_ANCHOR_LEVEL_PIX,
                                        moderatorLevel), size));
                        }
                    }
                    ...
                    if (!TextUtils.isEmpty(level)) {
                        ...
                        if (++count <= 10) {
                            spannableStringBuilder.append(" ").append(Utils
                                .getImageToSpannableString(mTvModels,
                                    Utils.getLevelImageResourceUri(Constants.USER_LEVEL_PIX, level),
                                    size));
                        } else {
                            // 否则  显示到第二行
                            spannableStringBuilder2.append(" ").append(Utils
                                .getImageToSpannableString(mTvModels2,
                                    Utils.getLevelImageResourceUri(Constants.USER_LEVEL_PIX, level),
                                    size));
                        }
                    ...
                    for (int i = 10; i < medalArray.length(); i++) {
                        ...
                        if (!TextUtils.isEmpty(url)) {
                            spannableStringBuilder2.append(" ")
                                .append(Utils.getImageToSpannableString(mTvModels2, url, size));
                        }
                    }
                    //主播等级
                    if (!TextUtils.isEmpty(moderatorLevel)) {
            ...
            //主播等级
            if (!TextUtils.isEmpty(moderatorLevel)) {
                spannableStringBuilder.append(" ").append(Utils.getImageToSpannableString(mTvModels,
            ...
            if (!TextUtils.isEmpty(level)) {
                spannableStringBuilder.append(" ").append(Utils.getImageToSpannableString(mTvModels,
                    Utils.getLevelImageResourceUri(Constants.USER_LEVEL_PIX, level), size));
            }
            mTvModels.setText(spannableStringBuilder);
            mTvModels2.setVisibility(View.GONE);
        }
        ...
        if (!TextUtils.isEmpty(bgImg)) {
            ImageLoaderUtil.getInstance()
                .loadImage(mListView.getRefreshableView().getHeaderView(), bgImg,
                    R.drawable.bg_fan_detail_head, R.drawable.bg_fan_detail_head);
            ImageLoaderUtil.getInstance().loadImage(mTopBackgroup, bgImg);
        }
        //		mHeadDynamicNum.setText(TextUtils.isEmpty(mPersonInfo.get("postNum")) ? "0" : mPersonInfo
        //				.get("postNum"));
        String fansNum =
            TextUtils.isEmpty(mPersonInfo.get("fansNum")) ? "0" : mPersonInfo.get("fansNum");
        ...
        if (AppConfig.getInstance().showGuard) {
            ...
            mHeadGuardNum.setText(
                TextUtils.isEmpty(mPersonInfo.get("guardNum")) ? "0" : mPersonInfo.get("guardNum"));
        } else {
            mHeadGuardLayout.setVisibility(View.GONE);
            mHeadGuardLine.setVisibility(View.GONE);
        }
        mHeadFocusNum.setText(TextUtils.isEmpty(mPersonInfo.get("attentionNum")) ? "0"
            : mPersonInfo.get("attentionNum"));
        ...
        if (isOwer) {
            ...
            //如果是主播/星探，并且正在开播
            if ((Constants.USER_TYPE_ANCHOR.equals(mPersonInfo.get("type"))
                || Constants.USER_TYPE_ROOMOWNER.equals(mPersonInfo.get("type")))) {
                ...
                if (isPlaying) {
                    ...
                    mHeadLiveFansNum
                        .setTextColor(getResources().getColor(R.color.a_text_color_ffffff));
                    mHeadAnchorLevelTip
                        .setTextColor(getResources().getColor(R.color.a_text_color_ffffff));
                    ...
                    mHeadLiveFansNum
                        .setTextColor(getResources().getColor(R.color.a_text_color_999999));
                    mHeadAnchorLevelTip
                        .setTextColor(getResources().getColor(R.color.a_text_color_666666));
                ...
                mHeadLiveFansNum.setText(
                    String.format(mActivity.getString(R.string.person_anchor_fans), fansNum));
            } else {
                //如果不是自己不是主播、星探
                mHeadLiveLayout.setVisibility(View.GONE);
            }
    ...
    private void initListView(LayoutInflater inflater) {
        ...
        mListView
            .setOnRefreshListener(new PullToRefreshBase.OnRefreshListener2<PullToZoomListView>() {
    ...
    public void onClick(ViewGroup parent, View v, int position, AlbumBean data) {
        ...
        intent.putExtra(ImageBrowserActivity.INIT_SHOW_INDEX,
            mAlbumViewPager.getCurrentItem() * 8 + position);
        ...
        public void onCallback(boolean success, String errorCode, String errorMsg, Object result) {
            ...
            if (success) {
                try {
                    ...
                    // 数据库存储用户个人信息
                    if (result != null) {
                        DatabaseUtils.saveOrupdatePersonInfoToDatabase(App.mContext,
                            JacksonUtil.readValue(String.valueOf(result), PersonInfo.class));
                        //						DatabaseUtils.getPersonInfoByUid(mActivity, mPersonUid);
                    }
                    // 如果fragment未回收，发送消息
                    if (meFragment != null) {
