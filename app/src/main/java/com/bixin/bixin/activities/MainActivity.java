...
import com.lib.common.utils.JsonUtil;
...
public class MainActivity extends BaseFragmentActivity implements OnClickListener {
	...
	private AMapLocationListener locationListener = new AMapLocationListener() {
		...
		public void onLocationChanged(AMapLocation aMapLocation) {
			...
			if (MainActivity.this.mDestoryed || hasUpdatePos)
				return;
			// 获取位置信息成功
			if (aMapLocation.getErrorCode() == 0 && Utils.isNetAvailable(MainActivity.this)) {
				hasUpdatePos = true;
				BusinessUtils.postUserPosition(MainActivity.this, String.valueOf(aMapLocation.getLongitude()),
	...
	private BroadcastReceiver netWorkChangeReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			if (ConnectivityManager.CONNECTIVITY_ACTION.equals(intent.getAction()) && Utils.isNetAvailable(MainActivity.this)) {
				updateUserLocation();
			}
		}
	};
...
//		updateMeInfoNum();
	...
	public boolean onKeyDown(int keyCode, KeyEvent event) {
		if (keyCode == KeyEvent.KEYCODE_BACK && event.getAction() == KeyEvent.ACTION_DOWN) {
			...
			// 按返回键弹出对话框
			UiHelper.showConfirmDialog(MainActivity.this, R.string.a_main_exit_message, R.string.logout_cancel,
			...
			public void onTabChanged(int tabIndex) {
				...
				if (tabIndex == TAB_ME) {
					...
					if (!AppConfig.getInstance().isLogged) {
						ActivityJumpUtil.gotoActivityForResult(MainActivity.this, LoginActivity.class,
...
//				Utils.setCfg(mActivity, Constants.USER_SF_NAME, mUserInfo);
	...
	private void checkNetwork() {
		if (!TelephoneUtil.isNetworkAvailable()) {
			UiHelper.showConfirmDialog(MainActivity.this, R.string.network_setting_msg, R.string.settings,
			...
			public void onCallback(boolean success, String errorCode, String errorMsg, Object result) {
				if (success) {
					try {
						...
						UserInfoConfig config = JsonUtil.getInstance().fromJson(result.toString(), UserInfoConfig.class);
	...
	private void showMoreWindow(View view) {
		...
		BusinessUtils.getLiveStatus(mActivity, new LiveStatusCallbackDataHandle(MainActivity.this));
	}
