...
public class LiveNearFragment extends BaseFragment implements OnClickListener, OnItemClickListener {
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		OperationHelper.onEvent(App.mContext, "clickBbroadcasterSImgInNearbyTab", null);
	...
	private void initListView(View v, LayoutInflater inflater) {
		...
		mPullRefreshGridView.getRefreshableView().setColumnWidth((
            App.metrics.widthPixels - Utils.dpToPx(LiveNearListAdapter.Uniform_Space) * 3) / 2);
		...
		mPullRefreshGridView.setOnRefreshListener(new OnRefreshListener2<GridView>() {
			...
			public void onPullDownToRefresh(PullToRefreshBase<GridView> refreshView) {
				OperationHelper.onEvent(App.mContext, "refreshInNearbyPageOfIndex", null);
				// 加载Banner,anchor列表
				reRequestData(false);
			}
