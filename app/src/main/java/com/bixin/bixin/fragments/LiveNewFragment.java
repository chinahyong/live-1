...
public class LiveNewFragment extends BaseFragment implements OnClickListener, OnItemClickListener {
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		MobclickAgent.onEvent(App.mContext, "clickBbroadcasterSImgInNewTab");
		OperationHelper.onEvent(App.mContext, "clickBbroadcasterSImgInNewTab", null);
	...
	private void initGridView(View v) {
		...
		mGridView.setColumnWidth((
            App.metrics.widthPixels - Utils.dpToPx(LiveNewListAdapter.Uniform_Space) * 4) / 3);
		refreshLayout.setOnRefreshListener(new CustomRefreshLayout.OnRefreshListener() {
			@Override
			public void onRefresh() {
				OperationHelper.onEvent(App.mContext, "refreshInNewPageOfIndex", null);
				// 加载Banner,anchor列表
				reRequestData(false);
			}
