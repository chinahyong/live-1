...
import tv.live.bx.R;
import com.bixin.bixin.activities.AnchorSearchActivity;
...
public class LiveFragment extends BaseFragment implements OnClickListener {
	...
	public final static int TAB_RECOMMEND = 10; //推荐
	/**
	 * 顶部Bar TAB栏
	 */
	...
	protected void initWidgets() {
		...
		mTabPagerIndicator.setOnTabReselectedListener(new TabPageIndicator.OnTabReselectedListener() {
			...
			public void onTabReselected(int position) {
				...
				if (TAB_NEW == index) {
					MobclickAgent.onEvent(App.mContext, "clickNewButtonInIndex");
					OperationHelper.onEvent(App.mContext, "clickNewButtonInIndex", null);
				} else if (TAB_HOT == index) {
					MobclickAgent.onEvent(App.mContext, "clickFeatureButtonInIndex");
					OperationHelper.onEvent(App.mContext, "clickFeatureButtonInIndex", null);
				} else if (TAB_RECOMMEND == index) {
					MobclickAgent.onEvent(App.mContext, "clickRecommendButtonInIndex");
					OperationHelper.onEvent(App.mContext, "clickRecommendButtonInIndex", null);
				} else if (TAB_NEAR == index) {
					MobclickAgent.onEvent(App.mContext, "clickNearbyButtonInIndex");
					OperationHelper.onEvent(App.mContext, "clickNearbyButtonInIndex", null);
				} else {
					Map<String, String> map = new HashMap<>();
					map.put("tabId", String.valueOf(data.get(LiveFragmentStatusAdapter.ID)));
					MobclickAgent.onEvent(App.mContext, "clickClassificationButtonInIndex");
					OperationHelper.onEvent(App.mContext, "clickClassificationButtonInIndex", map);
				}
	 ...
	 *
	 * @see com.bixin.bixin.fragments.BaseFragment#onActivityCreated(android.os.Bundle)
	 */
	@Override
	...
	public void onClick(View v) {
		switch (v.getId()) {
			case R.id.live_bar_search_btn:
				MobclickAgent.onEvent(App.mContext, "searchOnIndex");
