...
/**
 * @author Live
 * @version 2.4.0 2016.5.9
 * @title LiveFocusFragment.java Description:主播列表页"关注主播正在直播列表"
 */
public class LiveFocusFragment extends BaseFragment implements OnClickListener, OnItemClickListener {
	...
	private AnchorListAdapter mAuthorAdapter;
	 ...
	 *
	 * @see com.bixin.bixin.fragments.BaseFragment#onTabSelected()
	 */
	@Override
	...
	private void initListView(View v, LayoutInflater inflater) {
		...
		mAuthorAdapter = new AnchorListAdapter(mActivity);
		// 主播类别标签的点击事件
		...
		mListView.setTask(new Runnable() {
			@Override
			public void run() {
				OperationHelper.onEvent(App.mContext, "refreshInFollowPageOfIndex", null);
				// 请求主播数据
				reRequestData(false);
			}
		});
	...
	private void requestPlazaData(int page) {
		BusinessUtils.getAuthorListData(mActivity, page, null, LiveFragment.LIVE_STATUS_FOCUS, 0,
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		try {
			OperationHelper.onEvent(App.mContext,"clickBbroadcasterSImgInFollowTab", null);
		...
		switch (msg.what) {
			...
			case MSG_LOAD_SUCCESS:
				...
				if (!TextUtils.isEmpty(String.valueOf(objects[2]))) {
					mTvEmptyMsg.setText(String.valueOf(objects[2]));
				}
