...
package com.bixin.bixin.ui.widget;
...
public class HeaderFooterGridView extends GridView {
	...
	private List<OnScrollListener> a_listener = new ArrayList<OnScrollListener>();
	...
	private void initHeaderGridView() {
		// 设置它的子控件可以超出自身边界绘制，默认为true则不能超出边界
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (adapter != null && adapter instanceof HeaderFooterViewGridAdapter) {
			((HeaderFooterViewGridAdapter) adapter)
	...
	public void addHeaderView(View v, Object data, boolean isSelectable) {
		...
		if (adapter != null
				&& !(adapter instanceof HeaderFooterViewGridAdapter)) {
	...
	private OnScrollListener onScrollListener = new OnScrollListener() {
		...
		public void onScrollStateChanged(AbsListView view, int scrollState) {
			for (int i = 0; i < a_listener.size(); i++) {
				a_listener.get(i).onScrollStateChanged(view, scrollState);
			}
		}
		...
		public void onScroll(AbsListView view, int firstVisibleItem,
			...
			for (int i = 0; i < a_listener.size(); i++) {
				a_listener.get(i).onScroll(view, firstVisibleItem,
	...
	public boolean removeHeaderView(View v) {
		if (mHeaderViewInfos.size() > 0) {
			...
			ListAdapter adapter = getAdapter();
			if (adapter != null
					&& ((HeaderFooterViewGridAdapter) adapter).removeHeader(v)) {
				result = true;
			}
	...
	public boolean removeFooterView(View v) {
		if (mFooterViewInfos.size() > 0) {
			...
			ListAdapter adapter = getAdapter();
			if (adapter != null
					&& ((HeaderFooterViewGridAdapter) adapter).removeFooter(v)) {
				result = true;
			}
	...
	public void setAdapter(ListAdapter adapter) {
		if (mHeaderViewInfos.size() > 0 || mFooterViewInfos.size() > 0) {
			HeaderFooterViewGridAdapter hAdapter = new HeaderFooterViewGridAdapter(
	 ...
	 */
	private static class HeaderFooterViewGridAdapter implements
		...
		public HeaderFooterViewGridAdapter(
		...
		public int getCount() {
			if (mAdapter != null) {
				...
				return (getHeadersCount() * mNumColumns) + mAdapter.getCount()
						+ emptyItemCount + (getFootersCount() * mNumColumns);
			} else {
		...
		public boolean isEnabled(int position) {
			...
			if (position < numHeadersAndPlaceholders + mAdapter.getCount()
					+ emptyItemCount + numFootersAndPlaceholders) {
				return (position % mNumColumns == 0)
						&& mFooterViewInfos.get((position
								- numHeadersAndPlaceholders
								- mAdapter.getCount() - emptyItemCount)
		...
		public Object getItem(int position) {
			...
			if (position < numHeadersAndPlaceholders + mAdapter.getCount()
					+ emptyItemCount + numFootersAndPlaceholders) {
				if (position % mNumColumns == 0) {
					return mFooterViewInfos
							.get((position - numHeadersAndPlaceholders
									- mAdapter.getCount() - emptyItemCount)
									/ mNumColumns).data;
				}
			}
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			if (position < numHeadersAndPlaceholders + mAdapter.getCount()) {
				...
				if (mAdapter != null) {
					adapterCount = mAdapter.getCount();
					if (adjPosition < adapterCount) {
						convertView = mAdapter.getView(adjPosition,
								convertView, parent);
			...
			if (position < numHeadersAndPlaceholders + mAdapter.getCount()
				...
				// View height.
				convertView = mAdapter.getView(mAdapter.getCount() - 1,
						convertView, parent);
				convertView.setVisibility(View.INVISIBLE);
				return convertView;
			}
