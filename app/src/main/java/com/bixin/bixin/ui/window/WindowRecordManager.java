package com.bixin.bixin.ui.window;
...
public class WindowRecordManager {
	...
	public void createSmallWindow(Context context) {
		EvtLog.e(TAG, "createSmallWindow");
		...
		if (mWindowRecordSmall == null) {
			...
			if (recordSmallWindowParams == null) {
				...
				boolean permission = (PackageManager.PERMISSION_GRANTED == pm.checkPermission("android.permission.SYSTEM_ALERT_WINDOW", App.mContext
                    .getPackageName()));
				if (permission) {
					recordSmallWindowParams.type = WindowManager.LayoutParams.TYPE_PHONE;//设置窗口的window type
				} else {
					recordSmallWindowParams.type = WindowManager.LayoutParams.TYPE_TOAST;//设置窗口的window type
				}
	...
	public void removeSamllWindow(Context context) {
		EvtLog.e(TAG, "removeSamllWindow");
		if (mWindowRecordSmall != null) {
			WindowManager windowManager = getWindowManager(context);
	...
	public void destroySmallWindow(Context context) {
		EvtLog.e(TAG, "destroySmallWindow");
		if (mWindowRecordSmall != null) {
			WindowManager windowManager = getWindowManager(context);
			if (mWindowRecordSmall.isShown()) {
				//移除悬浮窗口
		...
		if (mWindowRecordBig == null) {
			...
			if (recordBigWindowParams == null) {
				...
				if (permission) {
					...
					recordBigWindowParams.type = WindowManager.LayoutParams.TYPE_TOAST;//设置窗口的window type
				}
