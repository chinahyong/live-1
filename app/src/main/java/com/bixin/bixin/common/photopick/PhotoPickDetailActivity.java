package com.bixin.bixin.common.photopick;
...
//		getSupportActionBar().setDisplayHomeAsUpEnabled(true);
		...
		if (mAllPhotos == null) {
			...
			if (!folderName.isEmpty()) {
				where = String.format("%s='%s'", MediaStore.Images.ImageColumns.BUCKET_DISPLAY_NAME, folderName);
			}
			mCursor = getContentResolver().query(MediaStore.Images.Media.EXTERNAL_CONTENT_URI,
					new String[]{MediaStore.Images.ImageColumns._ID, MediaStore.Images.ImageColumns.DATA}, where, null,
	...
	class ImagesAdapter extends FragmentStatePagerAdapter {
		...
		public Fragment getItem(int position) {
			...
			bundle.putString(ImagePagerFragment.IMAGE_URL, ImageInfo.pathAddPreFix(path));
