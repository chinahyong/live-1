...
public class MeFragment extends BaseFragment implements OnClickListener, OnRefreshListener {
	 ...
	 */
	private RelativeLayout mSettingLayout, mStore, mBackpack,
			mIncomeLayout, moLlAdvice, mLlLevel, mLlAnchorLevel, mSettingAccoutLayout, mLlRedPackage;
	private View mSettingAccoutDivider, mIncomeLine;
	...
	private TextView moTvNickname, moTvCoinBalance, mIncomeTitle, mTvIncome,
			moTvUserId, moTvEdit, mHeadFansNum, mHeadGuardNum, mHeadFocusNum, mTvVerifyInfo, mTvLevel, mTvAnchorLevel, mTvIntroduction,
			mTvNewProducts;
	private LinearLayout mHeadFansLayout, mHeadFocusLayout, mHeadGuardLayout;
	...
	protected void initMembers() {
		...
		mHeadFansLayout = (LinearLayout) mRootView.findViewById(R.id.item_fans_layout);
		mHeadFansNum = (TextView) mRootView.findViewById(R.id.item_fans_num);
		mHeadGuardLayout = (LinearLayout) mRootView.findViewById(R.id.item_guard_layout);
		mHeadGuardLine = mRootView.findViewById(R.id.item_guard_line);
		mHeadGuardNum = (TextView) mRootView.findViewById(R.id.item_guard_num);
		mHeadFocusLayout = (LinearLayout) mRootView.findViewById(R.id.item_focus_layout);
		mHeadFocusNum = (TextView) mRootView.findViewById(R.id.item_focus_num);
		...
		mSettingAccoutLayout = (RelativeLayout) mRootView.findViewById(R.id.me_setting_account_layout);
		mSettingAccoutDivider = mRootView.findViewById(R.id.me_setting_account_divider);
		...
		swipeRefreshLayout.setColorSchemeResources(R.color.a_bg_color_da500e, android.R.color.holo_green_light, android.R.color.holo_orange_light, android.R.color.holo_red_light);

	}

	@Override
	protected void initData(Bundle bundle) {
		if (!Utils.getBooleanFlag(Utils.getCfg(mActivity, Constants.COMMON_SF_NAME, GUIDE_EDIT,
				"false"))) {
			showFullDialog(R.layout.dialog_guide_edit_layout, new DialogInterface.OnDismissListener() {
				@Override
	...
	protected void setEventsListeners() {
		...
		mSettingAccoutLayout.setOnClickListener(this);
	}
...
//			mTvAnchorLevel.setText(String.valueOf(UserInfoConfig.getInstance().moderatorLevel + 1));
		...
		SpannableString sp = new SpannableString(mActivity.getResources().getString(R.string.me_income_tip) + "  (微信提现)");
		int start = mActivity.getResources().getString(R.string.me_income_tip).length();
		sp.setSpan(new ForegroundColorSpan(0xffaaaaaa), start, sp.length(), Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		sp.setSpan(new AbsoluteSizeSpan(14, true), start, sp.length(), Spanned.SPAN_EXCLUSIVE_INCLUSIVE);
		...
		if (!Utils.isStrEmpty(lsCoin))
			//#ec4c09--上一版本的颜色
			moTvCoinBalance.setText(Html.fromHtml(mActivity.getResources().getString(R.string.me_balance_text) + "<font color='#aaaaaa'> " + lsCoin + "</font>"));
		/**
		 * 个人主页：动态、粉丝、关注 转移到这里
		 * @version 2.5.0
		 */
		mHeadFansNum.setText(UserInfoConfig.getInstance().fansNum + "");
		if(AppConfig.getInstance().showGuard){
			mHeadGuardLayout.setVisibility(View.VISIBLE);
			mHeadGuardLine.setVisibility(View.VISIBLE);
			mHeadGuardNum.setText(UserInfoConfig.getInstance().guardNum + "");
		}else{
			mHeadGuardLayout.setVisibility(View.GONE);
			mHeadGuardLine.setVisibility(View.GONE);
		}
		mHeadFocusNum.setText(UserInfoConfig.getInstance().attentionNum + "");
		//判断是否账户保护
		if (!TextUtils.isEmpty(UserInfoConfig.getInstance().mobile)) {
			mSettingAccoutLayout.setVisibility(View.GONE);
			mSettingAccoutDivider.setVisibility(View.GONE);
		} else {
			mSettingAccoutLayout.setVisibility(View.VISIBLE);
			mSettingAccoutDivider.setVisibility(View.VISIBLE);
		}
	...
	private void showGetPhotoDialog() {
		actionSheetDialog = new ActionSheetDialog(mActivity).builder().setCancelable(true).setCanceledOnTouchOutside(true).addSheetItem(getString(R.string.system_camera), ActionSheetDialog.SheetItemColor.BLACK, new ActionSheetDialog.OnSheetItemClickListener() {
			@Override
	...
	public void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		switch (requestCode) {
			...
			case PhotoSelectImpl.REQUEST_CROP:
				...
				break;
			case PhoneBindActivity.REQUEST_PHONE_BIND_CODE:
	...
	public void onClick(View v) {
		switch (v.getId()) {
			...
			case R.id.my_info_anchor_level:
				...
				ActivityJumpUtil.gotoActivity(mActivity, WebViewActivity.class, false, WebViewActivity.WEB_INFO, (Serializable) webAnchorInfo);
				break;
			case R.id.me_setting_account_layout:
			...
			case R.id.my_info_redpackage_layout:
				ActivityJumpUtil.toWebViewActivity(mActivity, WebConstants.getFullWebMDomain(WebConstants.REDPACKET_WEB_URL), true);
				break;
			default:
				break;
		}
