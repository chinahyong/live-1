 ...
 */
public class GroupSubjectFragment extends BaseFragment implements OnClickListener, OnItemClickListener {
	public static final String GROUP_FORUM = "group";
	/**
	 * 栏目信息
	 */
	...
	private GroupSubjectListAdapter mSubjectAdapter;
	...
	public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
		...
		final Map<String, Object> lmItem = (Map<String, Object>) mSubjectAdapter.getItem(position
				- mListView.getHeaderViewsCount());
		EvtLog.d(TAG, "lmItem " + lmItem.toString());
		if (GroupSubjectListAdapter.TYPE_POST.equals(lmItem.get("type"))) {
			...
			fanInfo.put("id", (String) lmItem.get("group_id"));

			if (Utils.getBooleanFlag(lmItem.get("joined"))) {
				ActivityJumpUtil.gotoActivity(mActivity, FanDetailActivity.class, false, FanDetailActivity.FAN_INFO,
						(Serializable) fanInfo);
			} else {
				// 弹出对话框
				UiHelper.showConfirmDialog(mActivity,
						mActivity.getResources().getString(R.string.live_fan_unjoned_tip),
						R.string.commutity_fan_menber_status_add, R.string.cancel,
						new DialogInterface.OnClickListener() {
							@Override
							public void onClick(DialogInterface dialog, int which) {
								addFan((String) lmItem.get("group_id"));
							}
						}, new DialogInterface.OnClickListener() {
	...
	protected void handleMessage(Message msg) {
		switch (msg.what) {
			case MsgTypes.MSG_FAN_SUBJECT_FAILED:
			...
			case MsgTypes.MSG_FAN_SUBJECT_SUCCESS:
	...
	private void initListHeadData() {
		...
		if ("0".equals(mPostMoudle.get("id")) || GROUP_FORUM.equals(mPostMoudle.get("type"))) {
	...
	private void initListView(View v, LayoutInflater inflater) {
		...
		mSubjectAdapter = new GroupSubjectListAdapter(mActivity, new SupportOnclick());
		// 设置是否显示栏目标题
		mSubjectAdapter.setIsShowFroum(true);
	...
	private void requestPlazaData(String forumId, String isHot, int page) {
		if (SubjectInfo.HOT.equals(isHot)) {
			// BusinessUtils.getForumSubjectHotListData(mActivity, page,
			// forumId,
			// new CommutityCallbackData(forumId, isHot));
		} else {
			BusinessUtils.getGroupSubjectListData(mActivity, page, new CommutityCallbackData(forumId, isHot));
		}
	}
	...
	private class SupportOnclick implements IOnclickListener {
		...
		public void onClick(View v, int position, TextView numTextV) {
			...
			v.setSelected(true);
			v.setTag("true");
			BusinessUtils.groupSupport(mActivity, new SupportCallbackData(GroupSubjectFragment.this),
	...
	private class CommutityCallbackData implements CallbackDataHandle {
		...
		public void onCallback(boolean success, String errorCode, String errorMsg, Object result) {
			...
			if (success) {
				try {
					msg.what = MsgTypes.MSG_FAN_SUBJECT_SUCCESS;
					...
					// 发送完消息之后再保存数据，保存数据耗时
					// if (isRefresh) {
					// DatabaseUtils.saveListSubjectInfos((List<Map<String,
					// Object>>) objects[1], forumId, isHot);
					// }
					if (SubjectInfo.HOT.equals(isHot)) {
				...
				msg.what = MsgTypes.MSG_FAN_SUBJECT_FAILED;
				msg.obj = errorMsg;
				sendMsg(msg);
			}
		}
		...
		public void onCallback(boolean success, String errorCode, String errorMsg, Object result) {
			...
			if (success) {
				try {
					...
					// 下次请求的页面数
					GroupSubjectFragment fragment = (GroupSubjectFragment) mFragment.get();
					// 如果fragment未回收，发送消息
					if (fragment != null)
				...
				msg.what = MsgTypes.MSG_SUPPORT_FAILED;
