...
//			loHolder.mGiftCharts = (ImageView) convertView.findViewById(R.id.gifts_bottom_info_img_is_band_bg);
		...
		if (TextUtils.isEmpty(giftInfo.get("cornerMark"))) {
			...
			ImageLoaderUtil.with().loadImage(mContext, loHolder.giftTypeBg, giftInfo.get("cornerMark"));
		}
		...
		if (!TextUtils.isEmpty(giftInfo.get("name"))) {
			...
			ImageLoaderUtil.with().loadImage(mContext, loHolder.giftPhoto, giftInfo.get("imgPreview"));
	...
	private void setFlowerProgress(float pfPercent, ImageView moVProgress) {
		Bitmap loBmp = Bitmap.createBitmap(Utils.dip2px(mContext, 60), Utils.dip2px(mContext, 60), Config.ARGB_8888);
