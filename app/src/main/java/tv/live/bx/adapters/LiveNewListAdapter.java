...
public class LiveNewListAdapter extends BaseAdapter {
	...
	public LiveNewListAdapter(Context poContext) {
		moContext = poContext;
		mImageHeight = (FeizaoApp.metrics.widthPixels - Utils.dpToPx(Uniform_Space) * 4) / 3;
	...
	public View getView(final int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			loHolder.mTvLocation = (TextView) convertView.findViewById(R.id.item_tv_location);
		...
		ImageLoaderUtil.with().loadImageTransformRoundedCorners(moContext, loHolder.moIvPhoto, (String) subjectInfo.get("headPic"), Constants.COMMON_DISPLAY_IMAGE_CORNER_3);
		...
		loHolder.mTvLocation.setText(location);
		return convertView;
	}

	class Holder {
		private ImageView moIvPhoto;
		private TextView mtvTitle, mTvNumber, mTvLocation;
	}
