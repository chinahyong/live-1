...
public class AudienceAdapter extends LZBaseAdapter<Map<String, String>> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			...
			loHolder.moIvPhoto = (ImageView) convertView.findViewById(R.id.item_fm_rank_field_photo);
		...
		ImageLoaderUtil.with().loadImage(moContext, loHolder.mUserLevel, Utils.getLevelImageResourceUri(lmItem, true));
		loHolder.moIvPhoto.setImageResource(R.drawable.bg_user_default);

		if (!lstype.equals("-1") && lsPhoto != null)
			ImageLoaderUtil.with().loadImageTransformRoundCircle(moContext, loHolder.moIvPhoto, lsPhoto);
