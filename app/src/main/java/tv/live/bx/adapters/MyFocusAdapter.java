...
public class MyFocusAdapter extends MyBaseAdapter<String, Object> {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		if (convertView == null) {
			...
			loHolder.mIsPlayingLayout = (RelativeLayout) convertView.findViewById(R.id.isPlaying_layout);
		...
		if (!TextUtils.isEmpty((String) lmItem.get("moderatorLevel"))) {
			ImageLoaderUtil.with().loadImage(mContext, loHolder.mUserLevel, Utils.getLevelImageResourceUri(Constants.USER_ANCHOR_LEVEL_PIX,
					(String) lmItem.get("moderatorLevel")));
		}
		loHolder.mIsPlayingLayout.setVisibility(Utils.getBooleanFlag(lmItem.get("isPlaying"))
				? View.VISIBLE
				: View.GONE);
