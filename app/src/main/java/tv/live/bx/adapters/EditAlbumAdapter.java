...
public class EditAlbumAdapter extends RecyclerView.Adapter<EditAlbumAdapter.ImageHolder> implements ItemTouchCallbackListener.ItemTouchAdapter {
	...
	public void onBindViewHolder(ImageHolder holder, int position) {
		...
		if (!TextUtils.isEmpty(url)) {
			...
			ImageLoaderUtil.with().loadImageTransformRoundedCorners(mContext, holder.mIvPhoto, url, Constants.COMMON_DISPLAY_IMAGE_CORNER_1);
		} else {
			...
			if (!TextUtils.isEmpty(path)) {
				...
				ImageLoaderUtil.with().loadImageTransformRoundedCorners(mContext, holder.mIvPhoto, path, Constants.COMMON_DISPLAY_IMAGE_CORNER_1);
			}
		}
	}
	...
	public void onMove(int fromPosition, int toPosition) {
		EvtLog.e("EditAlbumAdapter", "onMove:fromPosition:" + fromPosition + "  toPosition:" + toPosition);
		// 当位置为0时，禁止移动
