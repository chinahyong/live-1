...
public class GroupSubjectListAdapter extends BaseAdapter {
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            loHolder.mPictureLayout = (LinearLayout) convertView
                .findViewById(R.id.item_picture_grid);
            ...
            loHolder.mSupportLayout = (RelativeLayout) convertView
                .findViewById(R.id.item_support_layout);
            convertView.setTag(loHolder);
        } else {
            loHolder = (Holder) convertView.getTag();
        }
        ImageLoaderUtil.getInstance()
            .loadHeadPic(moContext, loHolder.moIvPhoto, (String) subjectInfo.get("headPic"));
        ...
        try {
            ...
            for (int i = 0; i < jsonarray.length(); i++) {
                if (i < Constants.SUBJECT_LIST_IMAGE_LIMIT) {
                    LinearLayout.LayoutParams params = new LayoutParams(Constants.IMAGE_WIDTH,
                    ...
                    imageview.setOnClickListener(new OnClickListener() {
                        ...
                        public void onClick(View v) {
                            ActivityJumpUtil
                                .toImageBrowserActivity(moContext, (Integer) v.getTag(), imageUrl);
                        }
                    });
                } else {
                    imageUrl.add((String) jsonarray.get(i));
        ...
        if (isShowFroum) {
            ...
            loHolder.mForumTitle.setText(String
                .format(moContext.getResources().getString(R.string.commutity_catelory_text),
        ...
        if (!TextUtils.isEmpty((String) subjectInfo.get("user_level"))) {
            ImageLoaderUtil.getInstance().loadImage(loHolder.mUserLevel, Utils
                .getLevelImageResourceUri(Constants.USER_LEVEL_PIX,
                    (String) subjectInfo.get("user_level")));
        }
        if (!TextUtils.isEmpty((String) subjectInfo.get("last_reply_time"))) {
            loHolder.moTvTimer.setText(DateUtil.fmtTimemillsToTextFormat(
                Long.parseLong((String) subjectInfo.get("last_reply_time"))));
        }
        // 显示标题
        loHolder.mTitle.setText(HtmlUtil.htmlTextDeal(moContext, (String) subjectInfo.get("title"),
            new NetworkImageGetter(loHolder.mTitle,
                (int) (FeizaoApp.metrics.widthPixels - (16 * 2) * FeizaoApp.metrics.density)),
            null));
        ...
        String replaceImage = ((String) subjectInfo.get("content"))
            .replaceAll(Constants.REGX_PHOTO, Constants.REPLACE_PHOTO);
        loHolder.moTvContent.setText(HtmlUtil.htmlTextDeal(moContext, replaceImage,
            new NetworkImageGetter(loHolder.moTvContent,
                (int) (FeizaoApp.metrics.widthPixels - (16 * 2) * FeizaoApp.metrics.density)),
            null));
        ...
        loHolder.mSupportLayout.setOnClickListener(new OnClickListener() {
            ...
            public void onClick(View v) {
                if (cliOnclickListener != null) {
                    cliOnclickListener
                        .onClick(v.findViewById(R.id.item_support), position, loHolder.moTvSupport);
                }
            }
        });
