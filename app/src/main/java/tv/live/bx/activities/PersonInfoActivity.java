...
public class PersonInfoActivity extends BaseFragmentActivity implements OnClickListener,
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            ...
            case R.id.person_focus_layout:
                MobclickAgent.onEvent(FeizaoApp.mContext, "followInpersonalPage");
                // 如果已经关注了，则取消关注
                if (Utils.strBool(mPersonInfo.get("isAttention"))) {
                    ...
                    OperationHelper.onEvent(FeizaoApp.mContext, "followInpersonalPage", null);
            ...
            case R.id.item_live_photo_layout:
                MobclickAgent.onEvent(FeizaoApp.mContext, "liveInpersonalPage");
    ...
    protected void handleMessage(Message msg) {
        switch (msg.what) {
            ...
            case MsgTypes.FOLLOW_SUCCESS:
                //关注当前用户
                OperationHelper.onEvent(FeizaoApp.mContext, "followInpersonalPageSuccessful", null);
                mIsAttention = true;
                /** 返回关注此用户的状态*/
            ...
            case MsgTypes.REMOVE_FOLLOW_SUCCESS:
                //取消关注当前用户
                OperationHelper
                    .onEvent(FeizaoApp.mContext, "clickCancelFollowBroadcasterInpersonalPage",
                        null);
                mIsAttention = false;
                /** 返回关注此用户的状态*/
    ...
    private void initUI(LayoutInflater inflater) {
        ...
        mTopMargin = (int) (FeizaoApp.metrics.widthPixels * 0.7f - getResources()
    ...
    private void initAlbumListData(ViewPager viewPager, IconPageIndicator indicator,
        ...
        if (giftsData.size() > 0 && giftsData.size() <= 4) {
            ...
            // 图片高度 + 图片上下间距
            scale = (FeizaoApp.metrics.widthPixels - Utils.dip2px(mActivity, 5) * 5) / 4 * 2 + Utils
                .dip2px(mActivity, 5);
            indicator.setVisibility(View.GONE);
        ...
        // 有相册列表
        if (scale > 0) {
            mListView.getRefreshableView().setHeaderViewSize(FeizaoApp.metrics.widthPixels,
                (int) (FeizaoApp.metrics.widthPixels * 0.66 + scale));
        } else {
            //无相册列表
            mListView.getRefreshableView().setHeaderViewSize(FeizaoApp.metrics.widthPixels,
                (int) (FeizaoApp.metrics.widthPixels * 0.53));
        }
        ...
        public void onCallback(boolean success, String errorCode, String errorMsg, Object result) {
            ...
            if (success) {
                try {
                    ...
                    // 数据库存储用户个人信息
                    if (result != null) {
                        DatabaseUtils.saveOrupdatePersonInfoToDatabase(FeizaoApp.mContext,
