 ...
 * @Date 2016.6.14
 */
public class UserDynamicActivity extends BaseFragmentActivity implements OnClickListener, OnItemClickListener {
	...
	protected void initData(Bundle savedInstanceState) {
		...
		if (intent != null) {
			mPersonUid = intent.getStringExtra(USER_ID);
			isOwer = intent.getBooleanExtra(UserDynamicActivity.IS_OWER, false);
		}
		// 加载本地缓存数据
	...
	protected void initTitleData() {
		mTopTitleTv.setText(R.string.person_me_dynamic);
	...
	private void initListView(LayoutInflater inflater) {
		...
		mListView.setTopHeadHeight(Utils.dip2px(mActivity, 12));
...
//        mLoadProgress.Start(getResources().getString(R.string.a_progress_loading));
	...
	private void requestPlazaData(int page) {
		BusinessUtils.getPersonSubjectListData(mActivity, page, mPersonUid, new CollectCallbackData(this));
	}
	...
	protected void handleMessage(Message msg) {
		switch (msg.what) {
			...
			case MsgTypes.MSG_COLLECT_LIST_SUCCESS:
				...
				// 设置没有数据的EmptyView
				String text = mActivity.getString(R.string.person_no_dynamic_tip);
								...
								R.string.cancel, R.string.determine, new DialogInterface.OnClickListener() {
									...
									public void onClick(DialogInterface dialog, int which) {
										BusinessUtils.deleteGroupPost(mActivity, (String) subjectInfo.get("id"),
												new DeletePostCallbackData(UserDynamicActivity.this, position));
									}
								});
	 ...
	 * @since JDK 1.6
	 */
	private class CollectCallbackData implements CallbackDataHandle {
