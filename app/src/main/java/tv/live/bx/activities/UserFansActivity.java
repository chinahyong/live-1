 ...
 * 用户的粉丝
 */
public class UserFansActivity extends BaseFragmentActivity implements OnItemClickListener {
	/**
	 * 目前暂时都刷新
	 */
	...
	protected void initTitleData() {
		if (isOwer) {
			mTopTitleTv.setText(R.string.person_me_fans);
		} else {
			mTopTitleTv.setText(R.string.person_ta_fans);
		}
	...
	protected void handleMessage(Message msg) {
		...
		switch (msg.what) {
			...
			case MsgTypes.GET_FOCUS_USER_INFO_SUCCESS:
				...
				// 设置没有数据的EmptyView
				String text = mActivity.getString(R.string.person_ta_fans_no_data);
			...
			case MsgTypes.REMOVE_FOLLOW_SUCCESS:
				int removePosition = Integer.parseInt(msg.obj.toString());
...
//                final int position = (int) Integer.parseInt((String) v.getTag());
				/**
				 * @date 2016.6.8
				...
				if (Utils.strBool(mListAdapter.getData().get(position).get("isAttention"))) {
					BusinessUtils.removeFollow(mActivity,
							new RemoveFollowCallbackData(UserFansActivity.this, position),
							mListAdapter.getData().get(position).get("id"));
				} else {
					BusinessUtils.follow(mActivity, new FollowCallbackData(UserFansActivity.this, position),
							mListAdapter.getData().get(position).get("id"));
				}
			}
		});
	...
	private void requestPlazaData(int page) {
		BusinessUtils.getFansListData(mActivity, new UserInfoCallbackData(this), page, mUserId);
	}
