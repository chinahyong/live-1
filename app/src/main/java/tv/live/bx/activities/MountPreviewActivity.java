...
public class MountPreviewActivity extends BaseFragmentActivity implements View.OnClickListener {
	...
	public void initWidgets() {
		...
		mUserEntryBackground = (RelativeLayout) mUserEntryLayout.findViewById(R.id.item_user_entry_bg);
		...
		mUserGuardEntryText = (TextView) mUserEntryLayout.findViewById(R.id.item_user_guard_entry_text);
	...
	private void userEntryEffectHandle() {
		...
		mUserEntryLevelAlphaAnimation.setAnimationListener(new Animation.AnimationListener() {
			...
			public void onAnimationEnd(Animation animation) {
				...
				SpannableString loMountName = new SpannableString(
						mountName);
				loMountName.setSpan(new ForegroundColorSpan(0xfffff000), 0, loMountName.length(),
						Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
				charSequence = SpannableStringBuilder.valueOf(nickname).append(" ").append(mountAction).append(loMountName).
		...
		if (mUserEntryEffectTranAnimation == null) {
			mUserEntryEffectTranAnimation = AnimationUtils.loadAnimation(mActivity, R.anim.anim_live_user_effect_trans);
		...
		if (mUserEntryLayoutTransAnimation == null) {
			mUserEntryLayoutTransAnimation = AnimationUtils.loadAnimation(mActivity, R.anim.anim_live_user_entry_trans);
		}
		...
		mUserEntryEffect.setVisibility(View.INVISIBLE);
		ImageLoaderUtil.with().loadImage(mActivity, mUserEntryLevel, Utils.getLevelImageResourceUri(Constants.USER_LEVEL_PIX, level));
