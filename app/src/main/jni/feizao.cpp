...
static void invokeHandler(const char *body, unsigned int nBodySize,
		struct RTMP *_rtmp) {
	char *copyBody = (char*) malloc(nBodySize);
	if (!copyBody) {
//		LOGI("invokeHandler malloc1 error");
	...
	AMFObject *obj = (AMFObject*) malloc(sizeof(AMFObject));
	if (!obj) {
...
//	LOGI(AMFObject_STRING(obj, 0));
	...
	&& strcmp(AMFObject_STRING(obj, 0), "onInitRoom") == 0) {
		...
		int i = 0;
//		for(;i < sobj->o_num; i ++){
		AMFObject *item = AMFObject_OBJECT(sobj, 0);
//		LOGI("Nickname is:%s", AMFObject_STRING(item, 2));
...
//		int sex = AMFObject_NUMBER(sobj, 3);
//		int type = AMFObject_NUMBER(sobj, 4);
//		char* headPic = AMFObject_STRING(sobj, 5);
		int tmp = uid;
//		LOGI("Size of uid:%d", tmp);
//		else if (uid > 0) {
//			LOGI("UID > 0");
//		} else
//			LOGI("UID <= 0");
...
//		LOGI("%s LOGGED OUT", nickname);
	...
	&& strcmp(AMFObject_STRING(obj, 0), "onChatMsg") == 0) {
		AMFObject *sobj = AMFObject_OBJECT(obj, 5);
		char* from = AMFObject_STRING(sobj, 0);
		char* fromHead = AMFObject_STRING(sobj, 1);
		char* fromNickname = AMFObject_STRING(sobj, 2);
		char* msg = AMFObject_STRING(sobj, 3);
//		int private = AMFObject_BOOL(sobj, 4);
		char* to = AMFObject_STRING(sobj, 6);
		char* toHead = AMFObject_STRING(sobj, 7);
		char* toNickname = AMFObject_STRING(sobj, 8);
	}
	AMF_Reset(obj);
	free(copyBody);
}
...
//	LOGI("Rtmp Init success");
	...
	char* _url = (char *) pEnv->GetStringUTFChars(pUrl, NULL);
...
jint naSendMsg(JNIEnv *pEnv, jobject pOjb, jstring psTo, jstring psMsg,
	...
	AVal val0;
	val0.av_val = (char *) pEnv->GetStringUTFChars(psMsg, NULL);
	...
	AVal val1;
	val1.av_val = (char *) pEnv->GetStringUTFChars(psTo, NULL);
...
jint naSetup(JNIEnv *pEnv, jobject pObj, int pWidth, int pHeight) {
	...
	sws_ctx = sws_getContext(codecCtx->width, codecCtx->height,
			...
			NULL, NULL, NULL);
	buffer = (const uint8_t*) malloc(width * height * 4);
...
//			LOGI("Decode video frame");
			...
			if (frameFinished) {
				...
				sws_scale(sws_ctx,
						(const unsigned char * const *) decodedFrame->data,
...
//			auCodecCtx->channels, SL_PCMSAMPLEFORMAT_FIXED_16);
	tmpAudioBuf = (char*) malloc(16384);
	tmpBufOffset = 0;
	pthread_create(&decodeThread, NULL, (void * (*)(void *))decodeAndRender, pEnv);
}
...
jint JNI_OnLoad(JavaVM* pVm, void* reserved) {
	...
	jclass ffmpegClass = env->FindClass("com/lonzh/lib/LZFFmpeg");
//Register methods with env->RegisterNatives.
	env->RegisterNatives(ffmpegClass, nm, 8);
