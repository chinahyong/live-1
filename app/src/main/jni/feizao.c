...
//		LOGE("Found video type %d", formatCtx->streams[i]->codec->codec_type);
...
//				vCodecCtx->pix_fmt, surfaceWidth, surfaceHeight,
...
//	}
//	if (audioStream != -1) {
//		AVCodec *pAuCodec = NULL;
//		AVDictionary *auOptionsDict = NULL;
...
//	}
	...
	(*pEnv)->CallStaticVoidMethod(pEnv, mJClass, mid, 1);
...
void decodeAndRender() {
	...
	while (!stop && av_read_frame(formatCtx, &packet) >= 0) {
		...
		if (formatCtx->streams[packet.stream_index]->codec->codec_type
				== AVMEDIA_TYPE_VIDEO) {
			...
			if (videoStream == -1) {
				...
				AVDictionary * optionsDict = NULL;
				// Get a pointer to the codec context for the video stream
				vCodecCtx = formatCtx->streams[packet.stream_index]->codec;
				// Find the decoder for the video stream
				pCodec = avcodec_find_decoder(vCodecCtx->codec_id);
				if (pCodec == NULL) {
					LOGE("Unsupported video codec!\n");
				...
				LOGI("Alloc v_sws_ctx finished");
				videoBuf = malloc(surfaceWidth * surfaceHeight * 4);
				avpicture_fill((AVPicture *) frameRGBA, videoBuf,
						AV_PIX_FMT_RGBA, surfaceWidth, surfaceHeight);
				videoStream = packet.stream_index;
			}
			// Decode video frame
		...
		} else if (formatCtx->streams[packet.stream_index]->codec->codec_type
				== AVMEDIA_TYPE_AUDIO) {
			//如果读取流数据时，audioStream就进行初始化
			if (audioStream == -1) {
				AVCodec *pAuCodec = NULL;
				AVDictionary *auOptionsDict = NULL;
				aCodecCtx = formatCtx->streams[packet.stream_index]->codec;
				pAuCodec = avcodec_find_decoder(aCodecCtx->codec_id);
				if (pAuCodec == NULL) {
					LOGE("Unsupported Audio codec!\n");
					playing = 0;
				...
				createBufferQueueAudioPlayer(aCodecCtx->sample_rate,
						aCodecCtx->channels, SL_PCMSAMPLEFORMAT_FIXED_16);
				LOGI("createBufferQueueAudioPlayer success");
				audioStream = packet.stream_index;
			}
...
// 1 初始化Rtmp
	...
	if (res == 0)
		...
		jmethodID mid = (*pEnv)->GetStaticMethodID(pEnv, mJClass, "videoLoaded",
				"(I)V");
		(*pEnv)->CallStaticVoidMethod(pEnv, mJClass, mid, 0);
		(*jvm)->DetachCurrentThread(jvm);
		LOGE("video Loaded fail");
	}
	isRelease = 0;
...
jint JNI_OnLoad(JavaVM* pVm, void* reserved) {
	...
	(*env)->RegisterNatives(env, ffmpegClass, nm, 6);
