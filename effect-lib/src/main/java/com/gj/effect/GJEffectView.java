...
import com.gj.effect.util.PhoneUtil;
...
public class GJEffectView extends RelativeLayout {
	private static final String TAG = GJEffectView.class.getSimpleName();
...
//		LayoutParams params = new LayoutParams(-1, -1);
		params.leftMargin = layer.getStartPosition()[0];
		params.topMargin = layer.getStartPosition()[1];
		final float d = mContext.getResources().getDisplayMetrics().density;

//		mContext.getResources().getDisplayMetrics().density = 1.7f;
		if (layer.getDensity() != -1f) {
			EvtLog.e(TAG,"addEffectLottie  getDisplayMetrics :  "+d+"  ------ layer.getDensity()： "+layer.getDensity());
			PhoneUtil.setDensity(mContext,(float) layer.getDensity());
//			mContext.getResources().getDisplayMetrics().density = (float) layer.getDensity();
		}
		try {
...
//					mContext.getResources().getDisplayMetrics().density = d;
					PhoneUtil.reDensity(mContext);
				}
			});
			...
			EvtLog.e(TAG,"addEffectLottie ---- "+e.getMessage());
//			mContext.getResources().getDisplayMetrics().density = PhoneUtil.phoneDensity;
			PhoneUtil.reDensity(mContext);
			e.printStackTrace();
		}
	}
